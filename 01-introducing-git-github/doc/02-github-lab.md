...
4. Klik op "Clone or download" (rechtsboven)
5. Klik op **Open in Desktop**
...
6. Kies een locatie waar je de *clone* wilt opslaan, en klik op **Clone**
...
7. Kies voor **Show in Explorer**, en bekijk waar de Git *repository* zich op schijf bevindt.

### Maak iets stuk

8. **Klik** binnen GitHub Desktop op de knop **Open in Visual Studio Code**
9. Open `Sales.sql`
10. Verwijder regel 10 (hier staat de tekst `);`)
11. Sla het bestand op, en open **Source Control** - het volgende pictogram:
...
12. Onder het kopje **Changes**, klik op het **plus-teken** naast de **Sales.sql**:
    * ![Stage Changes](img/vscode-stage-file.png)
    * Het bestand komt nu onder een nieuw kopje **Staged Changes** te staan
13. Voer een commit message in, en klik op het vinkje om de *commit* door te voeren.
...
14. Zorg ervoor dat de commit die je zojuist gedaan hebt ook op de server belandt.
15. Verifieer op de *GitHub website* dat je wijziging daadwerkelijk doorkomt.
16. Bekijk ook het knop "History". Als het goed is, zie je hier de *commit* die je zojuist gedaan hebt terug.
...
17. Open **GitHub Desktop**
18. Met de trainings-repository geopend, kies voor de tab *History*
19. Navigeer door eerdere wijzigingen in de *repository*, en bekijk de wijzigingen die door de tijd heen gemaakt zijn.
20. Selecteer de meest recente wijziging. Aan de rechterzijde zie je dat in deze commit:
    * (a) Binnen Sales.sql
    * (b) Regel 10 verwijderd is
    * (c) Regel 10 bevatte de tekst `);`
![Wijzigingen in GitHub Desktop](img/wijzigingen-in-github-desktop.png)

21. Onder **History**, **rechtsklik** op de **meest recente commit**.
22. Kies **Revert this commit**.

![Revert this commit - GitHub Desktop](img/github-desktop-revert-this-commit.png)

23. Synchroniseer de nieuwe lijst met commits richting de *blessed repo*, en verifieer dat de code nu weer correct staat.
...
24. Pas het bestand `Sales.sql` opnieuw aan, verwijder regel 10
25. Zet op regel 10 nu de tekst `/* ONGEWENSTE WIJZIGING */`
26. Pas nu ook het bestand `SalesViaSSIS.sql` aan. Voeg hier op regel 2 de tekst `/*GEWENSTE WIJZIGING*/` in.
27. Sla de bestanden op, en doe een commit. *Doe hier geen sync, we voeren dit buiten de blessed repo uit*
28. Bekijk de geschiedenis binnen **GitHub Desktop**, en verifieer dat je *commit* correct is doorgevoerd
29. Open **Git Bash** of **Git cmd**, en navigeer naar de map waar je repository staat (bijv. `C:\src\git-github-workshop`). Hier kun je de volgende commando's voor gebruiken:
...
30. Geef het commando `git log -3`. Dit geeft je een lijst met de drie laatste wijzigingen.
31. Noteer de eerste zes tekens van de hash van de meest recente (bovenste) commit (bijv. `bb371d`)
32. Haal nu één bestand terug uit een andere *commit*. Dit kan met het commando `git checkout`.
...
34. Bekijk in **Visual Studio Code** of **GitHub Desktop** of de bestanden `Sales.sql` en `SalesViaSSIS.sql` nu correct zijn
...
35. Zorg ervoor dat deze gecorrigeerde aanpassing naar GitHub gebracht wordt. Je kunt dit naar keuze vanaf de commandline doen, binnen GitHub Desktop of binnen Visual Studio Code.
